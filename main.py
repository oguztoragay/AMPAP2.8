...
instances = {21: (3, 7, [0, 1, 2], [19], [1, 0])}

for i in instances.keys():
    Wtotal = 10*(instances[i][0]-1);  Htotal = 10*(instances[i][1]-1);  ll = 1  # ll:Load magnitude
    w, h, fixed, load_node, load_values = instances[i]
    today = str(datetime.date.today())
    foldername = 'C:/Users/ozt0008/Desktop/rere/' + today + '/' + str(i) + '/21/'
    ...
    while not stop:  # While loop begins here and continues until stop == True
        ...
        remained_nodes, nna = evaluate_result(w, h)
...
# -------------------------------------------------------
# 21: (3, 7, [0, 1, 2], [19], [0, 1])
# 45: (5, 9, [0, 1, 2, 3, 4], [42], [0, 1])
# 77: (7, 11, [0, 1, 5, 6], [73], [0, 1])
