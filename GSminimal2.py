...
""" Modified on Dec 8 2023 to relax the AM manufacturability condition of the potential members - to answer the minor revision requirements. """
...
class Generate:  # Ground structure generator
    def __init__(self, wt, ht, fix_nodes, load_nodes, load_values, Wtotal, Htotal, *args):
        ...
        for i, j in itertools.combinations(range(len(Nd)), 2):
            ...
            # if (angle == 90 and round(dy, 5) <= round((height / (ht - 1)), 5)) or ((angle == 45 or angle == 135) and round(dx, 5) <= round((width / (wt - 1)), 5)):
    ...
    def __init__(self, wt, ht, fix_nodes, load_nodes, load_values, ff, foldername, remained_nodes, tabu_list, nna, Wtotal, Htotal):
        ...
        for i, j in itertools.combinations(range(len(Nd)), 2):
            ...
            if gcd(int(dx), int(dy)) <= int(Wtotal / (wt - 1)):
                if (angle == 90 and round(dy, 5) <= round(Htotal / (ht - 1), 5)) or ((angle == 45 or angle == 135) and round(dx, 5) <= round(Wtotal / (wt - 1), 5)):
