...
def violation(PML, Nd, st, u, vol, a, itr, c_f):
    ...
    pro = UCn.keys()
    if itr != 1:
        remain_key = {k: v for k, v in a.items() if (v > 0.01)}.keys()
        remain_nodes_ = [[PML[i].nodei.name, PML[i].nodej.name] for i in remain_key]
        remain_nodes = set(chain.from_iterable(remain_nodes_))
        pro = {k: v for k, v in UCn.items() if (UCn[k].nodei.name in remain_nodes or UCn[k].nodej.name in remain_nodes) }.keys()  # and UCn[k].length < 200
    u = sum(u.values(), [])
    ...
    num = ceil(min(len(vioSort), 0.1*len(PML)))
